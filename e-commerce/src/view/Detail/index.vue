<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-for="(categoryName, index) in getName" :key="index">{{ categoryName }}
        </span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @keyup="skuNumKeyUp">
...
export default {
  ...
  methods: {
    ...
    addShopCart() {
      try {
        this.$store.dispatch('detail/addUpDateShopCart', {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum
        })
        this.$router.push({ name: 'addcartsuccess', query: { skuName: this.skuNum } })
        window.localStorage.setItem('skuInfo', JSON.stringify(this.skuInfo))
      } catch (error) {
        alert('加入购物车失败，请稍后重试~~~')
  ...
  watch: {
    categoryView() {
      // 获取 categoryView 中的 name信息
      // 监听属性 防止返回的是undefined 或者 {}
      const data = Object.keys(this.categoryView).filter(item => /^category\dName$/.test(item))
      let arr = []
      data.forEach(item => {
        arr.push(this.categoryView[item])
      });
      this.getName = arr
    }
  }
