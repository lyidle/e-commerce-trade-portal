  ...
  <div class="cart">
        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" minnum="1" class="itxt" :value="cart.skuNum"
              @change="handler('num', $event.target.value * 1, cart)">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck" @change="AllChecked">
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  methods: {
    ...
    // 修改某一个产品的个数 节流 1s
    handler: throttle(async function (single, v, cart) {
      {
        switch (single) {
          case 'plus':
            v = 1
          ...
          case 'num':
            ...
            v = (isNaN(v) || v < 1) ? 0 : parseInt(v) - cart.skuNum
            break;
        }
        try {
          // /代表的是修改成功
          await this.$store.dispatch('detail/addUpDateShopCart', { skuId: cart.skuId, skuNum: v })
          // 再一次获取服务器最新的数据进行展示
          this.getData()
        } catch (err) {
          alert(error.message)
    ...
    // 修改某个产品的勾选状态 防抖1s
    updateChecked: debounce(async function (cart, e) {
      try {
        await this.$store.dispatch('shopCart/UpdateCheckedById', { skuId: cart.skuId, isChecked: +e.target.checked })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
    ...
    // 完善全选框 防抖1s
    AllChecked: debounce(async function (e) {
      try {
        await this.$store.dispatch('shopCart/isAllChecked', +e.target.checked)
    ...
    // 删除选中产品 节流1s
    deleteAllCart: throttle(async function () {
      try {
